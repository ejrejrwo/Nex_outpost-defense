...
![Unreal C++ gameplay](Docs/Screenshots/combat.png)
...
native 2.0.0에서는 전투 중 채굴·강화·방벽 운반까지 한 흐름으로 연결합니다. 총·곡괭이·맨손은 준비·전투·휴식 중 전환할 수 있고, 전투 중 대장간 작업 중에도 적은 공격합니다. E로 취소하면 광석은 돌려받지만 이미 흐른 시간은 돌아오지 않습니다.
...
- `OutpostArenaRenderer.*`: Unreal Canvas로 아레나와 모든 월드 오브젝트를 절차적으로 렌더링
...
AI는 C++ 구현 보조, ImageGen 기반 레거시 아트 생성, 표준 라이브러리 기반 사운드 생성, 문서화와 검증 절차 보조에 참여했습니다. 게임 규칙과 제품 방향은 사람의 기획 판단으로 정했으며, 생성·보조 코드와 아트의 범위를 구분해 기록했습니다. 현재 PNG 아틀라스는 주 렌더링 경로에 사용하지 않는 레거시 자료입니다. [에셋 기록](Docs/ASSETS.md)과 [기술 개요](Docs/TECHNICAL_OVERVIEW.md)를 참고하십시오.

## 검증

최신 빌드·자동 테스트·GPU 플레이 결과는 [검증 기록](Docs/VALIDATION.md)에 버전과 함께 기록합니다. 자동 조작 결과는 실제 사람 플레이의 승률을 뜻하지 않습니다.
